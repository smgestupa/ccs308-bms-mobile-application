<script lang="ts">
    ...
    const getBooks = async () => {
        const req = await fetch(`http://localhost:8080/api/v1/books/search${ query ? `?query=${query}` : ""}`, {
            headers: {
...
<div class="px-4 py-20 h-full flex-grow">
    <div class="join w-full mt-4">
        ...
        <button class="btn join-item rounded-r-full" on:click={getBooks}>
